Приношу извинения за ответ на английском языке. Сейчас я подготовлю подробное описание сервиса валидации фотографий на русском языке, избегая конструкций с ключевой мыслью и двоеточием. Я использую более естественный стиль повествования, как если бы документ писал живой человек.

# Сервис валидации фотографий

## Обзор

Сервис валидации фотографий представляет собой специализированное веб-приложение для автоматической проверки цифровых фотографий на соответствие ряду критериев качества. В составе решения имеется API для загрузки изображений, их анализа согласно настраиваемым правилам и получения детальных результатов проверки.
...
|---------------------|-------------|
| Параметры файла | Проверка формата JPEG и соответствия размера файла установленным ограничениям |
...
| Тело запроса | Форм-данные с полем `file`, содержащим изображение в формате JPEG |
...
|----------|-------------------------|-------------------|-------------------|-----------------|
| `fileFormat` | Файл должен быть в формате JPEG | Проверка расширения файла (`.jpg`, `.jpeg`) | `PASSED` | `"jpeg"` |
...
1. Поместите тестовые изображения в формате JPEG в директорию `tests/photos/`.
2. Перейдите в директорию со скриптом: `cd tests/scripts`.
3. Запустите скрипт:
   ```bash
   python check_photos.py [параметры]
   ```

Скрипт последовательно отправляет все фотографии из указанной директории на валидацию, ожидает результаты (с повторными попытками) и формирует комплексный HTML-отчет в директории `tests/reports/`. Отчет включает превью изображений, итоговые статусы и детали по каждой проверке. После завершения скрипт пытается открыть отчет в браузере по умолчанию.
...
| `--max-attempts` | Максимальное количество попыток получения результата на один запрос | 20 |
| `--delay` | Задержка между попытками получения результата (секунды) | 3 |
...
2. Создайте класс, наследующий от `app.cv.checks.base.BaseCheck`.
3. Определите атрибуты класса: `check_id` (уникальный идентификатор), `name`, `description`, `default_config` (параметры по умолчанию).
4. Реализуйте метод `run(self, image: np.ndarray, context: Dict[str, Any]) -> Dict[str, Any]`. Метод должен возвращать словарь с ключами `status`, `reason` (опционально), `details`.
5. При следующем запуске сервиса новая проверка будет автоматически зарегистрирована и добавлена в `checks_config.yaml` (если её там нет), после чего её можно будет включить и настроить.

### Управление моделями (models/)

Директория `models/` предназначена для хранения файлов моделей машинного обучения. Путь к ней задается переменной окружения `MODELS_DIR`. Сервис настроен на автоматическую загрузку определенных моделей (YuNet, DNN, LBF) при их отсутствии. Убедитесь, что контейнер `app` имеет доступ к сети, если модели не предоставлены локально.

### База данных

Используется PostgreSQL. Схема определяется моделью `ValidationRequest` в `app/db/models.py` и управляется через Alembic (`migrations/`).

## Производительность

Сервис использует `asyncio` для обработки входящих запросов и постановки задач в очередь. Обработка изображений выполняется асинхронно в фоновом режиме (`app/worker/tasks.py`). Количество одновременно обрабатываемых изображений ограничено значением `MAX_CONCURRENT_PROCESSING` (см. `app/core/config.py`), что предотвращает перегрузку CPU и памяти.

Время обработки одной фотографии (`processingTime`) зависит от ее размера и сложности, а также от текущей нагрузки на сервер, и обычно находится в диапазоне от 0,5 до 3 секунд.
...
| Ошибка `Failed to decode image` | - Загруженный файл поврежден или не является корректным JPEG-изображением |
